...
test("When first player is not plafico and starts by bidding pacos, then there should be an error", () => {
  ...
  expect(() => game.bid(1, PerudoGame.DiceFace.Paco)).toThrow(
    PerudoGame.ErrorMessages.START_NON_PLAFICO_ROUND_BY_BIDDING_PACOS
  );
...
test("When first player is not plafico and starts by anything but pacos, then there should be no error", () => {
  ...
  for (
    let diceFace: PerudoGame.DiceFace = PerudoGame.DiceFace.Two;
    diceFace < PerudoGame.diceFacesNames.length;
    diceFace++
  ) {
    game.bid(1, diceFace);
  }
