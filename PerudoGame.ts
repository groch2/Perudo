...
enum RoundDiceOutcome {
  PlayerLostOneDice,
  PlayerRecoveredOneDice,
}
...
class Round {
  ...
  constructor(
    ...
    this.isFirstPlayerPalafico = nbDicesByPlayer[firstPlayerId] === 1;
  ...
  public bid(nbDices: number, diceFace: DiceFace) {
    const throwErrorWithMessageIfBiddingIsNotIncreasedByDiceFaceValueXOrByDiceQuantity =
      (errorMessage: string) => {
        ...
        if (
          !(
            (diceFaceDiff == 0 && diceQuantityDiff > 0) ||
            (diceFaceDiff > 0 && diceQuantityDiff == 0)
          )
        ) {
    ...
    if (this.isFirstPlayerPalafico) {
      ...
      if (this.isRoundBeginning) {
        if (diceFace === DiceFace.Paco) {
          throw new Error(
            ErrorMessages.START_NON_PLAFICO_ROUND_BY_BIDDING_PACOS
          );
        }
      } else {
        if (this.lastTurn.playerBidDiceFace !== DiceFace.Paco) {
          ...
          if (diceFace === DiceFace.Paco) {
            ...
            if (nbDices <= this.lastTurn.playerBidDiceQuantity * 2) {
              throw new Error(
                ErrorMessages.BIDDING_NON_PACOS_AFTER_A_BID_OF_PACOS
              );
            }
          }
        }
    ...
    this._nextPlayerId = getNextPlayerId(
      this.nbDicesByPlayer,
  ...
  public callBluff() {
    if (this.isRoundBeginning) {
      throw Error(
        ErrorMessages.CALLING_BLUFF_OR_EXACT_MATCH_AT_THE_BEGINNING_OF_THE_ROUND
      );
    }
    const countPacosAsJoker =
      this.lastTurn.playerBidDiceFace !== DiceFace.Paco &&
      !this.isFirstPlayerPalafico;
    const nbDicesMatchingLastBid = this.playersDicesDrawByPlayerId.reduce(
      (diceTotal, playerDices) => {
        return (
          diceTotal +
          playerDices.get(this.lastTurn.playerBidDiceFace) +
          (countPacosAsJoker ? playerDices.get(DiceFace.Paco) : 0)
        );
      },
      0
    );
    ...
    this._endOfRound = new EndOfRound(
      this.nextPlayerId,
      this.lastTurn.playerId,
      true,
      impactedPlayerId,
  ...
  public callExactMatch() {
    ...
    const isExactMatch =
      this.lastTurn.playerBidDiceQuantity === nbDicesMatchingLastBid;
  ...
  public get totalOfPositiveNbOfDicesByFaceName() {
    return [...this.totalNbDicesByFaceIndex.entries()]
      .map(([diceFaceIndex, diceQuantity]) => [
        diceFacesNames[diceFaceIndex],
        diceQuantity,
      ])
...
export class Game {
  ...
  public constructor(nbPlayersOrNbDicesByPlayer: number | number[]) {
    ...
    nbDicesByPlayer = nbDicesByPlayer.length
      ? nbDicesByPlayer
      : new Array(nbPlayersOrNbDicesByPlayer as number).fill(
          nbStartingDicesByPlayer
        );

    this._nbDices = nbDicesByPlayer.reduce((a, b) => a + b);

    this._nbPlayers = nbDicesByPlayer.length;
    this._nbPlayersNotEliminated = nbDicesByPlayer.filter(
      (nbDices) => nbDices > 0
    ).length;

    const playersDicesDrawByPlayerId = nbDicesByPlayer.map((nbStartingDices) =>
      getDrawByThrowingDices(nbStartingDices)
    );
  ...
  private initializeNewRound(): void {
    const nbDicesOfEachPlayerByPlayerId =
      this.currentRound.nbDicesByPlayer.splice(0);
    const playersDicesDrawByPlayerId = nbDicesOfEachPlayerByPlayerId.map(
      (nbDices) => getDrawByThrowingDices(nbDices)
    );
    const firstPlayerId = getNextPlayerId(
  ...
  private get currentRoundPlayersDicesDrawByPlayerId() {
    return this.currentRound.playersDicesDrawByPlayerId.map((diceDraw) =>
      [...diceDraw.entries()]
        .filter(([, quantity]) => quantity > 0)
        .map(([diceFace, diceQuantity]) => [
          diceFacesNames[diceFace],
          diceQuantity,
        ])
    );
  }
  ...
  public get currentBidNbDices() {
    return this._currentNbDicesBid;
  }

  private _currentDiceFaceBid: DiceFace | undefined = undefined;
  public get currentBidDiceFace() {
    return this._currentDiceFaceBid;
  }
}
...
export function getDrawByThrowingDices(nbDices: number): Map<DiceFace, number> {
  let nbDiceByFace = diceFacesNames.reduce(
    (nbDiceByFace, diceFace) =>
      nbDiceByFace.set(diceFacesNames.indexOf(diceFace), 0),
  ...
  new Array(nbDices)
    ...
    .forEach((diceFace) =>
      nbDiceByFace.set(diceFace, nbDiceByFace.get(diceFace) + 1)
    );
  return nbDiceByFace;
}
